<script lang="ts">
	...
	// for any load not yet priced.
	function loadValue(j: Load): number {
	...
	let loadsById = $derived(new Map(loads.map(j => [String(j.id), j])));
</script>
