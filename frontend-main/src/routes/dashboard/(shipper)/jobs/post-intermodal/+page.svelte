<script lang="ts">
	...
	async function postIntermodal() {
		...
		try {
			...
			postError = e.message || 'Failed to create intermodal job';
		} finally {
			postLoading = false;
		}
	}
	...
	<Row>
		<Column>
			<div class="page-header">
				<Button kind="ghost" size="small" href="/dashboard/jobs/post" icon={ArrowLeft}>
					Single-leg job instead
				</Button>
				<h1><Add size={24} /> Post an Intermodal Job</h1>
				<p class="sub">
		...
		<Column lg={11} md={8} sm={4}>
			...
			<div class="form-grid">
				...
				<TextInput bind:value={order.title}
					labelText="Job Title" placeholder="e.g. Dublin to Amsterdam door-to-door" />
				...
				<Button on:click={postIntermodal} disabled={postLoading || !order.title}>
					{postLoading ? 'Creating...' : 'Create Intermodal Job'}
				</Button>
			</div>
		</Column>
	</Row>
