...
- **Suggested build order:** (1) Postgres + migrations, (2) timetables on `CarrierLane` ✓ + transfer profiles on `Location` (open), (3) graph build ✓ (shipped — see "Routing-graph population" above) + single-criterion (cheapest) time-dependent search with deadline pruning (open — the next step here), (4) two-criteria Pareto (cost, CO2) + `EmissionPolicy`, (5) flexible-window loop + cross-day merge/dedupe, (6) options UI.
