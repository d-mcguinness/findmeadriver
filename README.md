...
- **Suggested build order:** (1) Postgres + migrations, (2) timetables on `CarrierLane` ✓ + transfer profiles on `Location` (open — `TransferPolicy` code defaults stand in meanwhile), (3) graph build ✓ + single-criterion (cheapest) time-dependent search with deadline pruning ✓ (both shipped — see the "Routing-graph population" / "Routing search v1" entries above), (4) two-criteria Pareto (cost, CO2) + `EmissionPolicy`, (5) flexible-window loop + cross-day merge/dedupe, (6) options UI.
