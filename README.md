...
- **Suggested build order:** (1) Postgres + migrations, (2) timetables on `CarrierLane` ✓ + transfer profiles on `Location` (open — `TransferPolicy` code defaults stand in meanwhile), (3) graph build ✓ + single-criterion (cheapest) time-dependent search with deadline pruning ✓, (4) two-criteria Pareto (cost, CO2) + `EmissionPolicy` ✓, (4b) **REST entry point ✓** — `POST /api/{shipper,admin}/route-options` (`RoutePlannerService.planRoutes` → `RouteOptionResponse[]`; read-only *propose*, nothing persisted yet), (5) flexible-window loop + cross-day merge/dedupe, (6) acceptance path (draft `Itinerary` from a chosen option via `CreateIntermodalLoadRequest`) + options UI.
