...
The multi-modal TMS build has **shipped**: road / rail / sea / air modes, intermodal itineraries (sequenced single-mode legs), per-mode rate-card pricing with per-mode commission, mode-aware credential matching, ROAD-only cabotage and EU-561 enforcement, and per-mode duty clocks (declared calendars + load-consumption netting).

The dev database is **H2 in-memory with `ddl-auto=create-drop`**, so there are no migrations today — the schema is recreated and reseeded on every restart. A production deployment would need **Flyway** migrations; all the multimodal columns are additive/nullable, so they are migration-friendly.
