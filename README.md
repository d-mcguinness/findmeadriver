...
- **Routing search v1 (build-order step 3, back half)** — `RoutePlanner.findOptions` is real: a time-dependent, label-setting cheapest search over one handover day (`earliestReady`, start-of-day origin-local), returning the single cheapest option that satisfies the arrival deadline (any time on the deadline day counts) — or, when nothing does, rerunning for speed alone and returning the fastest-possible option (its arrival past the deadline is the caller's signal, per the design goal). Mechanics: virtual road legs generated on demand (always to the query destination, plus feeders to in-radius terminals with onward service; great-circle × 1.3 circuity, 70 km/h; memoised per origin-location within a query), mode changes **and same-mode scheduled interchanges** (vessel→vessel etc.) gated by `TransferProfile` (absent = impossible; only road→road is free — one truck driving on), waiting for a later departure as a first-class branch via `nextDeparture`, A*-style deadline pruning (great-circle at 900 km/h — nothing beats a jet), **exact Pareto dominance** per `(location, arrivalMode)` bucket so the returned option is genuinely cost-optimal (the design's ~5% near-duplicate tolerance is deferred to step 4, where a Pareto *set* is returned — applying it to a single-option search lets an up-to-5%-pricier earlier label evict the true cheapest, compounding along a kill chain). Since the per-location transfer table hasn't landed, a **`TransferPolicy`** code-default bean (the `PricingPolicy` pattern) is compiled into the snapshot at build time: SEAPORT handles ROAD/RAIL/OCEAN, AIRPORT ROAD/AIR, RAIL/INLAND terminals ROAD/RAIL, plain ADDRESS nothing; transfer cost/dwell = the harder side's handling, and same-mode scheduled pairs (RAIL/OCEAN/AIR) are included so a shared-hub transshipment is costed. The real table will override these defaults. `serviceMode` PARCEL is rejected at lane creation alongside INTERMODAL (no routing-graph transfer support). Two-criteria Pareto (cost, CO2) is step 4; the flexible-window loop + cross-day merge is step 5 (`latestHandover` accepted, not yet explored).
