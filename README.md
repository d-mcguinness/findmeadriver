...
- **Suggested build order:** (1) Postgres + migrations, (2) timetables on `CarrierLane` ✓ + transfer profiles on `Location` (open — `TransferPolicy` code defaults stand in meanwhile), (3) graph build ✓ + single-criterion (cheapest) time-dependent search with deadline pruning ✓, (4) two-criteria Pareto (cost, CO2) + `EmissionPolicy` ✓, (4b) **REST entry point ✓** — `POST /api/{shipper,admin}/route-options` (`RoutePlannerService.planRoutes` → `RouteOptionResponse[]`), (5) flexible-window loop + cross-day merge/dedupe, (6) **acceptance path ✓** — `POST /api/shipper/route-options/accept` re-plans + matches the chosen option and materialises it as a draft PLANNED `Itinerary` via `createIntermodalLoad` (`AcceptRouteRequest` → `ItineraryResponse`); each leg then fills through the normal Load-per-leg marketplace. Remaining: admin on-behalf acceptance + options UI.
