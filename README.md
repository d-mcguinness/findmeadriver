...
- **Routing-graph population (build-order step 3, front half)** — the `com.driverdirect.routing` graph layer is implemented and wired; only the search (`RoutePlanner.findOptions`) remains a stub. Three decisions, made after an adversarial design review of the refresh question ("cron job or not"):
  - *Pattern-on-edge departures.* `ScheduledServiceEdge` holds the recurring weekly pattern (`Set<DayOfWeek>` + origin-local `LocalTime` + `ZoneId`) rather than a materialised departure list; `nextDeparture(after)` resolves each occurrence on the fly. Consequences: no planning horizon to decay (a materialised list silently returns "no service" past its edge — fatal for sea freight booked 2–3 months out), no daily roll-forward cron, and DST correct per occurrence (a stepped-Instant list drifts an hour after every transition; gap/overlap semantics are pinned in `ScheduledServiceEdgeTest`). The graph is therefore staled only by *writes*, never by the clock.
  - *Build-per-query freshness.* `RoutePlannerService.findOptions` builds a fresh `RoutingGraph` per route query — a build is two constant-count queries (fetch-joined timetabled lanes, all locations) plus map assembly, milliseconds at this network's size. No cache, no dirty flags, no `@EnableScheduling`, no startup ordering against the seed, and multi-instance production is correct by default. Named escape hatch if route volume ever forces it (~50+ QPS or >100 ms builds): a short-TTL memoisation around `build()` — still no scheduler.
  - *The snapshot closes over everything a search touches.* `RoutingGraph` carries the edges, a JPA-detached `LocationNode` index (coordinates + build-time-resolved zones, UTC fallback), and the road `Tariff` compiled from `PricingPolicy` (same `max(min, base + rate × qty)` formula as `PricingService`, incl. IATA volumetric kg) — so all searches of one query price one consistent world and never touch live repositories or the live policy bean mid-search. Edge `co2()` returns 0 until `EmissionPolicy` (step 4).
- **Routing search (build-order steps 3–4)** — `RoutePlanner.findOptions` is real: a time-dependent, label-setting search over one handover day (`earliestReady`, start-of-day origin-local), returning the **Pareto-best options on (cost, CO2)** that satisfy the arrival deadline (any time on the deadline day counts) — or, when nothing does, rerunning for speed alone and returning the single fastest-possible option (its arrival past the deadline is the caller's signal, per the design goal). Mechanics: virtual road legs generated on demand (always to the query destination, plus feeders to in-radius terminals with onward service; great-circle × 1.3 circuity, 70 km/h; memoised per origin-location within a query), mode changes **and same-mode scheduled interchanges** (vessel→vessel etc.) gated by `TransferProfile` (absent = impossible; only road→road is free — one truck driving on), waiting for a later departure as a first-class branch via `nextDeparture`, A*-style deadline pruning (great-circle at 900 km/h — nothing beats a jet).
  - *Dominance is split by design.* The search frontier uses **exact** Pareto dominance on `(cost, CO2, arrivalTime)` per `(location, arrivalMode)` bucket (`DOMINANCE_TOLERANCE = 0`) so the collected front is the true Pareto set — a non-zero tolerance in the *kill* direction evicts the true cheapest and compounds along a kill chain. The design's ~5% near-duplicate tolerance is applied **only to the returned front**, as a one-shot non-compounding ε-grid thinner (`OUTPUT_EPSILON = 0.05`), capped to `MAX_OPTIONS = 6` by even-sampling that keeps the cheapest and greenest endpoints. `findOptions` returns options cost-ascending; arrival is never an output axis (only a hard filter), so with no cargo weight (CO2 ≡ 0) the front correctly collapses to the single cheapest.
  - **`EmissionPolicy`** — code-default per-mode carbon intensity (kg CO2e per tonne-km, GLEC/ISO-14083 midpoints: OCEAN 0.012, RAIL 0.025, ROAD 0.075, AIR 0.800), the `PricingPolicy` pattern. Compiled into each edge at build time via **`LegRates`** (a `Tariff` + the CO2 factor, bundled so an edge carries both objective functions the Pareto search optimises); a leg's CO2 = `distance × tonnes × factor` on **actual** mass (not air chargeable weight — emissions track physical tonnage; the divergence from `PricingService`'s billing basis is documented at the seam). A per-lane emission table with validity periods would later override these defaults, exactly like the rate cards.
  - Since the per-location transfer table hasn't landed, a **`TransferPolicy`** code-default bean is compiled into the snapshot: SEAPORT handles ROAD/RAIL/OCEAN, AIRPORT ROAD/AIR, RAIL/INLAND terminals ROAD/RAIL, plain ADDRESS nothing; transfer cost/dwell = the harder side's handling, same-mode scheduled pairs (RAIL/OCEAN/AIR) included so a shared-hub transshipment is costed. `serviceMode` PARCEL is rejected at lane creation alongside INTERMODAL (no transfer support). The flexible-window loop + cross-day merge is step 5 (`latestHandover` accepted, not yet explored).
  - *Demo data.* The graph is only as good as what it's built from, so the seed now carries real coordinates + IANA timezones on every typed terminal (without them there are no road legs, leg distances are 0 → pricing/CO2 collapse, and A* pruning can't run) plus a dedicated line-haul operator ("EuroLink Multimodal") running competing continental services (ocean Dublin→Le Havre, rail Rotterdam→Lille). A seeded Dublin→Paris query then returns a genuine (cost, CO2) front — cheap-dirty direct road (~€1.3k, ~1140 kg CO2) vs pricier-greener sea+road via Le Havre (~€2.6k, ~380 kg) — verified by `RoutingSeedIntegrationTest`.
...
- **Graph, three layers, no graph database** — an in-memory adjacency-list graph (`Map<LocationId, List<ServiceEdge>>`) built *per route query* from the relational tables (build-per-query — see "Routing-graph population" above; a fresh snapshot costs milliseconds and is never cached); the query itself is a custom time-dependent Pareto search, not a stored graph.
  ...
  - *Service* — carrier services as scheduled edges. `CarrierLane` now carries an optional **timetable** (nullable, additive — landed as build-order step 2): `serviceMode` (one concrete mode; INTERMODAL rejected), optional typed-terminal anchors (`originLocation`/`destinationLocation`), and a recurring weekly pattern (`departureDays` + local `departureTime` + `transitDurationHours`), with `nextDeparture(after)` giving exactly the contract `ScheduledServiceEdge` needs. Untimetabled lanes stay pure country-pair browse preferences, unchanged; re-posting the same country pair upserts the schedule in place, and a plain re-add never wipes one. Timezone resolution landed with the graph build (zones resolved onto edges at build time from `Location.timezone`, documented UTC fallback). Still open: capacity/cargo constraints and explicit (non-recurring) departures. Road stays **virtual**: synthetic truck legs generated on demand between locations within a radius, priced by road distance — never stored as edges.
...
- **Suggested build order:** (1) Postgres + migrations, (2) timetables on `CarrierLane` ✓ + transfer profiles on `Location` (open — `TransferPolicy` code defaults stand in meanwhile), (3) graph build ✓ + single-criterion (cheapest) time-dependent search with deadline pruning ✓, (4) two-criteria Pareto (cost, CO2) + `EmissionPolicy` ✓, (4b) **REST entry point ✓** — `POST /api/{shipper,admin}/route-options` (`RoutePlannerService.planRoutes` → `RouteOptionResponse[]`), (5) **flexible-window loop ✓** — when `latestHandover` is after `earliestReady`, one search per candidate handover day (capped at 30, logged if truncated), merged across days keeping the latest viable handover per distinct route (a tighter `arrivalDeadline` caps how late); single-day queries are the one-element case, (6) **acceptance path ✓** — `POST /api/shipper/route-options/accept` re-plans + matches the chosen option and materialises it as a draft PLANNED `Itinerary` via `createIntermodalLoad` (`AcceptRouteRequest` → `ItineraryResponse`); each leg then fills through the normal Load-per-leg marketplace. Remaining: admin on-behalf acceptance, a shipper-facing "latest handover day" (vs the first-departure instant), + options UI.
