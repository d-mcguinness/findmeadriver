...
A time-dependent multi-criteria (cost, CO2) route planner that will *propose* itineraries instead of the shipper authoring every leg. The graph-population layer **and** the v1 search are implemented and wired; still open: Pareto on CO2 + `EmissionPolicy` (step 4), the flexible-window per-day loop (step 5, `RouteQuery.latestHandover` accepted but unexplored), and any REST exposure (step 6). The full design + build order live in root `README.md` ("Proposed: multimodal routing engine").
