...
### Routing engine (`com.driverdirect.routing`) — graph layer LIVE, search still a stub
A time-dependent multi-criteria (cost, CO2) route planner that will *propose* itineraries instead of the shipper authoring every leg. The **graph-population layer is implemented and wired** (build-order step 3's front half); the search (`RoutePlanner.findOptions()`) still throws `UnsupportedOperationException`. The full design + build order live in root `README.md` ("Proposed: multimodal routing engine") — read that before implementing the search.
...
- `TransferProfile` — mode-change cost/dwell keyed by `(location, fromMode, toMode)`; deliberately a lookup the search consults, **not** a graph edge (edges-as-transfers would force `(Location, Mode)` graph nodes). No table yet — the builder passes an empty map, which the search must read as "no transfer possible here", never free.
