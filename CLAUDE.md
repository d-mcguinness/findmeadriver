...
A time-dependent multi-criteria (cost, CO2) route planner that will *propose* itineraries instead of the shipper authoring every leg. The graph-population layer, the (cost, CO2) Pareto search, and a **REST entry point** are implemented and wired (`POST /api/{shipper,admin}/route-options` → `RoutePlannerService.planRoutes` → `RouteOptionResponse[]`). Still open: the flexible-window per-day loop (step 5, `RouteQuery.latestHandover` accepted but unexplored) and the **acceptance path** — turning a chosen `RouteOptionResponse` into a draft `Itinerary` via the existing `CreateIntermodalLoadRequest` flow (the README "integration point"; today the endpoint is read-only *propose*, nothing is persisted) — plus any frontend UI. The full design + build order live in root `README.md` ("Proposed: multimodal routing engine").
