...
### Routing engine (`com.driverdirect.routing`) — graph layer + cheapest search LIVE (build-order step 3 ✓)
A time-dependent multi-criteria (cost, CO2) route planner that will *propose* itineraries instead of the shipper authoring every leg. The graph-population layer, the (cost, CO2) Pareto search, and a **REST entry point** are implemented and wired (`POST /api/{shipper,admin}/route-options` → `RoutePlannerService.planRoutes` → `RouteOptionResponse[]`). The **acceptance path** is wired too (`POST /api/shipper/route-options/accept` → `RoutePlannerService.buildAcceptedItinerary` re-plans + matches the chosen option's leg sequence, maps its edges to `CreateIntermodalLoadRequest`, and posts through `createIntermodalLoad` → draft PLANNED `Itinerary`; each leg then fills via the normal Load-per-leg marketplace). Still open: the flexible-window per-day loop (step 5, `RouteQuery.latestHandover` accepted but unexplored), admin on-behalf acceptance, and the frontend UI. The full design + build order live in root `README.md` ("Proposed: multimodal routing engine").
- **Search v1 (`RoutePlanner.findOptions`)**: label-setting cheapest search from `earliestReady` (start of day, origin zone); returns the single cheapest option whose arrival makes the deadline (any time on the deadline day counts — `arrivalDeadline.plusDays(1)` exclusive), else reruns for speed and returns the fastest-possible option — arrival past the deadline is the caller's signal. Virtual road legs on demand (always to the query destination + feeders to terminals with onward service within 300 km; great-circle × 1.3 circuity at 70 km/h; memoised per origin-location); mode changes **and same-mode scheduled interchanges** (vessel→vessel etc.) need a `TransferProfile` at that location (absent = impossible; only road→road is free); dwell-for-departure is a first-class branch; A* deadline pruning at 900 km/h; **exact Pareto dominance** per `(location, arrivalMode)` bucket (`DOMINANCE_TOLERANCE = 0` — the design's ~5% belongs to step 4's multi-option search; a non-zero tolerance in a single-cheapest search evicts the true cheapest and compounds along kill chains). Constants are package-private statics on `RoutePlanner`. Throws `IllegalArgumentException` (→ 400) on unknown locations / same origin-destination / missing `earliestReady`; unreachable destination → empty list; a `MAX_LABEL_POPS` backstop warn-logs rather than silently truncating.
- **`TransferPolicy`** (`service`, code-default bean like `PricingPolicy`): per-terminal-type transfer capabilities compiled into the snapshot at build time (SEAPORT: ROAD/RAIL/OCEAN; AIRPORT: ROAD/AIR; RAIL/INLAND terminals: ROAD/RAIL; ADDRESS: none; cost/dwell = the harder side's handling; same-mode scheduled pairs RAIL/OCEAN/AIR included for shared-hub transshipment, ROAD self-pair excluded). Stand-in until the per-location transfer table (step 2's second half) lands and overrides it. `serviceMode` PARCEL is rejected at lane creation alongside INTERMODAL (no transfer support).
- **Populate strategy: build-per-query, no cache, no cron.** `RoutePlannerService.findOptions` builds a fresh `RoutingGraph` per route query (a build is 2 constant-count queries + map assembly — milliseconds at this network's size), so freshness is perfect by construction, all ~30 flexible-window searches of one query share one consistent method-local snapshot, and multi-instance production needs zero coordination. Deliberately absent: `@EnableScheduling`, dirty flags, invalidation events, startup builds. Named escape hatch if route volume ever forces it (~50+ QPS): short-TTL memoisation around `build()` — still no scheduler.
- `ServiceEdge` (interface) + `ScheduledServiceEdge` (rail/sea/air) + `RoadEdge` (virtual — generated on demand within a radius, never stored; costed from the snapshot's `roadTariff`). **`ScheduledServiceEdge` holds the recurring weekly *pattern*** (`Set<DayOfWeek>` + origin-local `LocalTime` + `ZoneId`), not a materialised departure list: `nextDeparture(after)` resolves each occurrence on the fly (≤8 candidate days), so the graph never goes stale with the passage of time — no planning horizon to decay, far-future queries just work, and DST resolves per occurrence (gap times shift forward, overlap takes the earlier offset — pinned in `ScheduledServiceEdgeTest`). Only *writes* (lane/location edits) stale a graph, and build-per-query absorbs those. `CarrierLane` carries the source timetable: nullable `serviceMode`/`originLocation`/`destinationLocation` + `departureDays` CSV/`departureTime`/`transitDurationHours`, `isTimetabled()`, `nextDeparture(after)` (origin-local). Untimetabled lanes stay pure country-pair browse preferences; `CarrierLaneService.add` upserts a schedule onto the existing (carrier, origin, destination) lane, rejects partial timetables and INTERMODAL serviceMode, and a plain country-pair re-add never wipes an existing schedule.
- `TransferProfile` — mode-change cost/dwell keyed by `(location, fromMode, toMode)`; deliberately a lookup the search consults, **not** a graph edge (edges-as-transfers would force `(Location, Mode)` graph nodes). No table yet — the builder compiles `TransferPolicy` defaults per typed terminal; a location with no profile pair means "no transfer possible here", never free.
- `RoutingGraph` — an immutable snapshot that **closes over everything a search touches**: adjacency lists (`Map<LocationId, List<ServiceEdge>>`), a `LocationNode` index (coordinates + zones, detached from JPA), and the compiled road `Tariff` — a search must never read live repositories or the live `PricingPolicy` mid-query. `RoutingGraphBuilder` is a `@Component` with `@Transactional(readOnly=true) build()`: one fetch-joined lane query (`CarrierLaneRepository.findTimetabledWithTerminals` — inner joins drop anchor-less lanes, constant query count) + one location query; zones resolved at build from `Location.timezone` (UTC fallback — seed locations declare none); unusable lanes are skipped fail-soft with a warn log. `Tariff` compiles `PricingPolicy.RateCard` to doubles via `Tariff.from(...)` (single mapping seam, like `CargoDetails.from`) and prices `max(min, base + rate × qty)` with per-unit quantity resolution incl. IATA volumetric kg; edge `co2()` returns 0 until `EmissionPolicy` (build-order step 4).
...
**Shipper (`/api/shipper/**`):** `loads` CRUD (`POST` / `GET` / `GET {id}` ownership-guarded / `PUT {id}` edit — OPEN-only) + `loads/{id}/status` + `loads/{id}/cancel`, `loads/{id}/applications` + accept/reject, `loads/{id}/rate`, `ratings`; **intermodal** `itineraries` (POST create / GET list / GET {id}); **routing** `POST route-options` (propose Pareto-best (cost, CO2) routes between two Location ids for a cargo — the `com.driverdirect.routing` engine's read-only entry; body `RouteQueryRequest`, returns `RouteOptionResponse[]`; unknown location / same origin-destination / missing `earliestReady` → 400, unreachable → `[]`) + `POST route-options/accept` (accept a proposed option — body `AcceptRouteRequest` echoes the chosen `RouteOptionResponse`'s legs; the server re-plans + matches that leg sequence and materialises it as a draft PLANNED `Itinerary` via `createIntermodalLoad`, returns `ItineraryResponse` (201); a selector no current option matches → 400). Tenant scoping applies to both (a shipper may only route between public reference nodes + its own locations).

**Admin (`/api/admin/**`):** `users` (+ `users/{id}/impersonate` — POST mints a login-shape token for any user so an admin can "mimic" them; mirrors `/api/user/login`, forbids self), `stats`, `shippers`, `carriers`, `loads` (POST on-behalf / GET / GET {id} / PUT {id} edit / cancel), `loads/{id}/carrier-eligibility`, `applications`, compliance review (`compliance/pending`, `compliance/{id}/verify`), read-only TMS views `orders`, `shipments`, `itineraries` (+ POST on-behalf), `locations`, and `POST route-options` (on-behalf mirror of the shipper route planner).
...
`admin@driverdirect.com` / `admin123` · `employer@company.com` / `employer123` (Shipper "Acme Logistics") · `driver@example.com` / `driver123` (multi-modal Carrier — supports ROAD/OCEAN/AIR, holds AIR:ATPL + OCEAN:STCW). The seed includes road/rail/sea/air loads, two intermodal itineraries, and typed port/airport/terminal Locations. **Routing-engine demo data:** the typed terminals carry real coordinates + IANA timezones (so the routing search has road legs, real leg distances/CO2, and A* pruning — without them steps 3–4 are inert), and a dedicated line-haul operator carrier (`ops@eurolink.example.com`, "EuroLink Multimodal") runs competing continental services — an ocean sailing Dublin Port → Le Havre and a Rotterdam → Lille rail leg — so a Dublin→Paris `RoutePlanner` query returns a genuine (cost, CO2) Pareto front (cheap-dirty direct road vs pricier-greener sea+road) rather than one option. Proven by `RoutingSeedIntegrationTest` (`@SpringBootTest`).
